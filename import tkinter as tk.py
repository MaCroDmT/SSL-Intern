...
def get_religious_holidays(year):
    ...
    """
    # Sample religious holidays. In a real-world scenario, you would
    # use a more reliable method (e.g., an API or a comprehensive library)
    ...
    if holidays_to_give > 0:
        ...
        if given_holiday_list:
            ...
            for holiday_name, holiday_date in final_list:
                ...
                output_text.insert(tk.END, f"- {holiday_name}: {holiday_date.strftime('%B %d, %Y')} ({season_info})\n")
    else:
...
year_entry.insert(0, str(datetime.datetime.now().year))  # Default to current year
